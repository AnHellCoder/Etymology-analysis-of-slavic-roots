...
from transformers import TrainingArguments, DataCollatorForSeq2Seq, T5ForSequenceClassification
...
def compute_metrics(eval_pred):
    ...
    # Если логиты представляют собой кортеж
...
for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(labels)), labels)):
    ...
    for i, w in enumerate(words):
        ...
        plt.title(f"ByT5 encoder mean attention for word {w}")
        plt.colorbar()

        plt.savefig(f"./etymology-analysis/ByT5_attentions/(PRETRAINED)ByT5_mean_attention_fold{fold+1}_word{i}.png")
    
    plt.close()
    #========== Конец визуализации усреднения распределения внимания по всем головам для предобуч. ByT5 ==========#


    #========== Визуализация распределений внимания по всем головам для предобуч. ByT5 на одном слове ==========#
    ...
    for h in range(num_heads):
        ...
        plt.savefig(f"./etymology-analysis/ByT5_attentions/(PRETRAINED)ByT5_attention_fold{fold+1}_head{h}.png")
        
    plt.close()
    #========== Конец визуализации распределений внимания по всем головам для предобуч. ByT5 на одном слове ==========#
    ...
    for i, w in enumerate(test_words):
        ...
        plt.xticks(range(len(tokens)), tokens, rotation=90)
        plt.yticks(range(len(tokens)), tokens)
        plt.title(f"ByT5 encoder mean attention for test word {test_words[i]}")
        plt.colorbar()

        plt.savefig(f"./etymology-analysis/ByT5_attentions/(PRETRAINED)ByT5_mean_attention_fold{fold+1}_testword{i}.png")
    ...
    #Приводим модели к требуемому для теста виду
    ...
    for i, w in enumerate(words):
        ...
        plt.savefig(f"./etymology-analysis/ByT5_attentions/ByT5_mean_attention_fold{fold+1}_word{i}.png")
    ...
    for h in range(num_heads):
        ...
        plt.savefig(f"./etymology-analysis/ByT5_attentions/ByT5_attention_fold{fold+1}_head{h}.png")
    ...
    for i, w in enumerate(test_words):
        ...
        plt.savefig(f"./etymology-analysis/ByT5_attentions/ByT5_mean_attention_fold{fold+1}_testword{i}.png")
